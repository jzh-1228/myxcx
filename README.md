...
当前版本接入了 **真实人像抠图 + 换底**、**弱美颜**（人像·美肤滑杆会改画布像素），以及 **目标 KB 压缩 + 过审本地预检**。滤镜 LUT、其它人像细项、AI 写真等仍为桩。
...
2. 切到 **合规**：滑杆上限变低。规格条「过审」会跑本地预检；若强度超过合规上限，可一键「恢复合规强度」。
3. 撤销 / 重做应回到上一张处理图。
4. 证件照：默认不自动美颜；在设置打开后再走一寸，结果应略弱于美颜模式。

## 目标 KB 压缩 + 过审检查

导出走 **设备内离屏 Canvas 重编码**，按目标体积对 JPEG 做质量二分，用 `wx.getFileInfo`（失败则读 ArrayBuffer 长度）量真实字节。透明底（已抠图且未换底）才保留 PNG，不会假装 JPEG 压到目标。

过审是 **本地预检**，文案固定为「本地预检，以受理机关要求为准」，不声称官方批准。

| 检查项 | 规则 |
|--------|------|
| 像素尺寸 | 对照所选规格 px（及 mm，若常量里有） |
| 画布比例 | 宽高比是否接近规格 |
| 底色均匀 | 仅在已抠图+换底时对边缘像素做启发式实色判断 |
| 文件体积 | 对照用户设定的目标 KB；未压缩先提醒，尽力后仍超标则未过 |
| 美颜强度 | 超过合规上限 35 记为提醒，可一键恢复 |
| 人脸存在 | **跳过**，不做人脸识别、不假装过检 |

### 建议点检路径（KB + 过审，微信开发者工具）

1. **修图** → 相册导入 → 顶栏 **导出**：选 **≤50KB** → **预览压缩**，应出现「预览 xxKB · JPEG xx%」。
2. 点 **过审检查**：无规格时尺寸/比例为跳过；体积按目标对照；美颜若超合规为黄档。
3. **保存相册**：相册文件应用 `getFileInfo` 量到的体积；能压到目标时 Toast 带最终 KB，压不到则弹窗说明尽力结果。
4. **证件照** → 一寸 → 选图进入编辑器。规格条 **KB检测** 打开同一导出板；**过审** 会按 295×413 / 25×35mm 对照。把美颜拉过 35 后再点过审，应出现「恢复合规强度」，点一下滑杆回到合规帽。
5. 自定义目标：导出板选「自定义」输入 `20`，预览后看是否 ≤20KB；极小目标看「尽力压缩」警告而不是假成功。
6. 透明底：抠图后不换底再导出，应保留 PNG，并说明无法用 JPEG 质量压到目标。

本地脚本可先验证压缩二分与清单逻辑（不替代开发者工具出图）：
...
services/compress.js   目标 KB（Canvas JPEG 二分）
services/audit.js      过审本地预检
services/ai.js         AI 生成桩（厂商无关）
constants/             含 export 预设 / specs 像素与 mm
...
| 撤销重做 | 含抠图结果路径的状态栈 | — |
| 保存相册 | 按目标 KB 重编码后 `wx.saveImageToPhotosAlbum`，展示实测 KB | — |
| 目标 KB 压缩 | 离屏 Canvas JPEG 质量二分；透明底保留 PNG | — |
| 过审检查 | 本地预检清单（绿/黄/红），非官方批准 | — |
...
4. 证件照：搜索或点「一寸」→ 选图 → 规格条可折叠，底栏仍是五态；规格条「过审 / KB检测」接真实预检与压缩。
5. 我的：作品 / 草稿 / 设置 / 隐私可进；设置可保存抠图地址。
6. 导出 Sheet：目标 KB 预设 / 自定义 → 预览压缩 → 过审检查 → 保存，预览区显示最终 KB。
...
结构校验 + 抠图解析 + 弱美颜像素/合规帽 + KB 二分数学 + 过审清单。不能替代微信开发者工具编译与真机点检。
